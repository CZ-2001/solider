...
#define IMUWarnBuzzerOn() buzzer_on(0, 0) //蜂鸣器静音（调试时过于刺耳）
...
#define INS_GET_CONTROL_TEMPERATURE() get_temprate()             //获取控制温度的目标值
...
static fp32 INS_PID[]=
{MPU6500_TEMPERATURE_PID_KP,MPU6500_TEMPERATURE_PID_KI,MPU6500_TEMPERATURE_PID_KD,MPU6500_TEMPERATURE_PID_MAX_IOUT,MPU6500_TEMPERATURE_PID_MAX_OUT};
...
//将读取到的ist8310原始数据处理成国际单位的数据
        ...
        if( mpu6500_real_data.status & 1 << MPU_DATA_READY_BIT)
        {
            ...
            if (updata_count == 0)
            {
               ...
               PID_Init(&imuTempPid, PID_DELTA, INS_PID);
